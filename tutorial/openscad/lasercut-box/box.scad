    ...
    spacing = 0)
{
  ...
  module left() { 
    cut_left() difference() {
      ...
      if (len(holes_left) > 0 )
        for (i = [ 0 : len(holes_left)-1 ])
          hole(holes_left[i], hole_left_dia);
    }
  }
  ...
  module front() {
    cut_front() difference() {
      ...
      if (len(holes_front) > 0)
        for (i = [ 0 : len(holes_front)-1 ])
          hole(holes_front[i], hole_front_dia);
      if (ears_radius > 0)
        ears_inner(true);
    }
  }
